...
TUTORIAL ON HOW TO USE: https://www.youtube.com/watch?v=IieEFJPrjN4

(This way I don't always have to update the command itself, 
the list, and the commentated list...)
...
local Settings = {
	...
	DisableAbuse = false;
	AbuseList = {"kill", "punish", "respawn", "smoke", "blind", "strobe", "flash", "control", "give", "health", "clone", "loopfling", "fling", "drug", "explode", "splode", "ragdoll", "change", "insert", "decal", "taketools", "removetools", "bomb", "say", "sink", "asteroid", "gun", "health", "damage", "btools", "missile", "reck", "truck"};
...
local Server = {
	...
	GDS = game:GetService("DataStoreService"):GetGlobalDataStore();
...
local ScriptCache,MarketCache,PackageCache = {},{},Packages
local TempChatLogs = {}
local ToolRegions = {Server.Lighting, game:GetService("ReplicatedStorage"), game:GetService("ServerStorage")}
local NoSpeak = false

local SOUND,MARKET,RANK,STRING,GUI = {},{},{},{},{}
local CORE,REMOTE = {Version = script:FindFirstChild("Version"), Domain = "http://www.classy-studios.com"},{RemoteAdmin = {Command, ServerCommand, ServerPort, TimeStamp}, RemoteConnection = false}
...
local RemoteData = {RemoteAdminWait = 5, ChatLogWait = 30}

-- import old settings just in case --
local OldSettings = script.Parent:FindFirstChild("LuaModelMaker's Admin Settings") or game:FindFirstChild("LuaModelMaker's Admin Settings", true)
if OldSettings then
	OldSettings = require(OldSettings)
	
	for _,ValName in pairs({"Ranks", "Prefix", "Bet", "ServerLocked", "DisableAbuse", "AbuseList", "AbusableGear", "LinkedAccount", "DataKey", "VIPMemberID", "VIPAdminID", "GroupID", "GroupMemberRank", "GroupAdminRank", "GroupOwnerRank"}) do
		Settings[ValName] = OldSettings[ValName]
	end
	Settings.GroupBanRank = OldSettings.RankBan
	Settings.Fun = OldSettings.FUN
end

if F3X then
	F3X = F3X:Clone()
	F3X.Handle.Anchored = false
	script:FindFirstChild("F3X Tools"):Destroy()
end

wait(0.1)

local SettingsValues = script.Parent:FindFirstChild("MakerModelLua's Admin Settings") or game:FindFirstChild("MakerModelLua's Admin Settings", true)
	...
	if SettingsValues:IsA("Folder") or SettingsValues:IsA("Configuration") then
		for _,Value in pairs(SettingsValues:GetChildren()) do
			if Settings[Value.Name] then
						...
						if ypcall(function() return NewValue.Value end) then
							if Settings[Value.Name][NewValue.Name] then
								...
								if type(Settings[Value.Name][NewValue.Name]) == "table" then
									Val = {}
									for Word in string.gmatch(NewValue.Value, "%w+") do
				...
				elseif ypcall(function() return Value.Value end) then
					if Settings[Value.Name] then
...
elseif game.PlaceId == 199055528 then
	Settings.LinkedAccount = "Admin"
end

local getfenv,setfenv,table,math,coroutine,pcall,ypcall,xpcall,string,assert,type,unpack,loadstring,tostring,tonumber = getfenv,setfenv,table,math,coroutine,pcall,ypcall,xpcall,string,assert,type,unpack,loadstring,tostring,tonumber -- localize variables so executables don't troll the script
...
for i = 1,5 do
	if GameOwner == "[ Client ]" then
		ypcall(function() GameOwner = Server.Players:GetNameFromUserIdAsync(game.CreatorId) end)
		print("MML's Admin: Attempting to establish owner's name #"..i)
		wait(1)
	end
end

if game.PlaceId == 21053279 or game.PlaceId == 21053219 or game.PlaceId == 20279777 or PlaceId == 260897989 then IsSB = true end

if IsSB then
	Settings.IRCServer = "swiftirc.net";
	Settings.IRCChannel = "#scriptbuilder";
...
for _,Font in pairs(Enum.Font:GetEnumItems()) do
	Fonts[string.sub(tostring(Font),11)] = Font
end

local LogClient = script:FindFirstChild("AddClientInfo")
if LogClient then LogClient:Destroy() end
LogClient = Instance.new("RemoteFunction", script) LogClient.Name = "AddClientInfo"
LogClient.OnServerInvoke = function(Player, Info)
	if Info then
		for Key,Data in pairs(Info) do
			ClientInfo[Player.Name][Key] = Data
		end
	end
end
...
function CORE:FixSettings()
	if Settings.Bet == "/" or Settings.Bet == "-" or Settings.Bet == "(" or Settings.Bet == ")" then Settings.Bet = " " end
...
function CORE:Execute(Source, Speaker)
	local Script, Error = loadstring([[local getfenv,setfenv,Commands,script = nil,nil,nil,nil    ]]..Source)
	if Error then
...
function CORE:RemoveAdmin(Speaker)
	...
	if not CORE:NilPlayer(Speaker) then
		Answer = GUI:PromptMessageWithButtons(Speaker, "Remove M".."akerModelLua's Admin?", "Would you like to remove Ma".."kerModel".."L".."uas's Admin? After removing the admin, no one is admin and cannot be re-inserted until the server restarts. Continue?", "Question", {"Yes", "No"})
	end
	if Answer == "Yes" or CORE:NilPlayer(Speaker) then
		for _,Player in pairs(Server.Players:GetPlayers()) do if Player:FindFirstChild("PlayerGui") then
			...
			for _,GUI in pairs(Player.PlayerGui:GetChildren()) do
				if GUI.Name == "M".."ak".."erModelLua's Admin Form" or GUI.Name == "M".."a".."k".."erMo".."delLua's TaskBar" or GUI.Name == "M".."a".."k".."e".."r".."m".."ode".."lLu".."a's HUD Gui" then
					GUI:Destroy()
				end
			end
		end end
...
function CORE:AddCommand(CommandNames, CommandExample, Description, Arguments, Rank, Fun, CommandFunction, IsAbusive)
	if IsAbusive == nil then IsAbusive = false end
	if Commands[CommandName] then table.remove(Commands[CommandName]) end
	Commands[CommandNames] = {CommandExample, Description, Arguments, Rank, Fun, CommandFunction, IsAbusive}
end
...
function CORE:ChangeName(Player, NewName)
	...
	if Player.Character ~= nil then if Player.Character:FindFirstChild("Humanoid") ~= nil then
		...
		Head.Transparency = 1
		pcall(function() Player.Character.Label:Destroy() end)

		pcall(function() Head.face.Transparency = 1 end)

		local Label = Instance.new("Model", Head.Parent)
		Label.Name = "Label"
...
function CORE:UpdateAdmin()
	...
	local NewAdminModel = Server.IS:LoadAsset(UpdateModel)
	...
	if AssetID then
		...
		if AbuseMode then
			for _,ID in pairs(RemoteAbuseGear) do
				if AssetID == ID then CanInsert = false break end
			end
...
function CORE:Fly(Player, Force)
	if Player and Player.Character then
		return CORE:ExecuteResource("Fly", Player.Character, {["Force"] = Force})
	end
end
...
function REMOTE:SendSnapshot(SendKey, MaxPartAmount, PrintResponse, Stopypcall)
				...
				if tonumber(Size[1]) >= MinSize.X and tonumber(Size[2]) >= MinSize.Y and tonumber(Size[3]) >= MinSize.Z then
					...
					if not Data.Shape and not Skip then
						if ypcall(function() return Obj.Shape end) then
							Data.Shape = string.sub(tostring(Obj.Shape),14)
					...
					if not Skip then
						...
						for _,Sub in pairs(Obj:GetChildren()) do
							if Sub:IsA("Decal") then
								...
								Data.Textures[FaceCovert[string.sub(tostring(Sub.Face),15)]] = Texture
							end
						end
	...
	local Res = "Could not get response"
	local function SendData()
		Res = Server.HS:PostAsync(CORE.Domain.."/Login/SendSnapshot.php?Username="..Settings.LinkedAccount.."&DataKey="..Server.HS:UrlEncode(Settings.DataKey).."&SendKey="..SendKey, PlaceJSON, 2)
	end
	if Stopypcall == true then SendData() else ypcall(SendData) end
	if PrintResponse then print(Res) end
...
function STRING:FormatReplace(Player, Text)
	Text = string.gsub(Text,"_SERVERTIME", math.floor(Workspace.DistributedGameTime))
	Text = string.gsub(Text,"_PLACEID", game.PlaceId)
	Text = string.gsub(Text,"_CREATORID", game.CreatorId)
	Text = string.gsub(Text,"_CREATOR", GameOwner)
	Text = string.gsub(Text,"_PLACENAME", PlaceName)
	Text = string.gsub(Text,"_REVPLACENAME", string.reverse(PlaceName))
	Text = string.gsub(Text,"_PLACEDESC", PlaceInfo.Description)
	Text = string.gsub(Text,"_RANDOM", math.random(1,10000))
	Text = string.gsub(Text,"_DATE", Date or "Cannot get date. Http not connected")
	Text = string.gsub(Text,"_TIME", CORE:GetTime(true))
	if Player then
		Text = string.gsub(Text,"_USERID", tostring(Player.userId))
		Text = string.gsub(Text,"_NAMELEN",tostring(string.len(Player.Name)))
		Text = string.gsub(Text,"_REVNAME",string.reverse(Player.Name))
		Text = string.gsub(Text,"_UPNAME",string.upper(Player.Name))
		Text = string.gsub(Text,"_LOWNAME",string.lower(Player.Name))
		Text = string.gsub(Text,"_NAME", Player.Name)
		Text = string.gsub(Text,"_ACCOUNTAGE", STRING:FindAge(Player.AccountAge))
		
		if Player.Character and Player.Character:FindFirstChild("Humanoid") then
			Text = string.gsub(Text,"_WALKSPEED", tostring(Player.Character.Humanoid.WalkSpeed))
			Text = string.gsub(Text,"_HEALTH", tostring(Player.Character.Humanoid.Health))
			Text = string.gsub(Text,"_MAXHEALTH", tostring(Player.Character.Humanoid.MaxHealth))
		end
		
		local PlayerRank,RankNum = RANK:GetRank(Player)
		Text = string.gsub(Text,"_RANKNUM", tostring(RankNum))	
		Text = string.gsub(Text,"_RANK", PlayerRank)
	end
	
	return STRING:Defilter(Text)
end
	...
	for _,Word in pairs(Words) do
		...
		elseif string.sub(Word,1,7) == "radius-" then
			...
			if DidExe then coroutine.wrap(function()
				...
				local Mesh = Instance.new("SpecialMesh", Part) Mesh.MeshType = "Sphere" Mesh.Scale = Vector3.new(NumArg, NumArg, NumArg)
...
function MARKET:PromptPurchase(Player, ProductID, From, BuyPressFunc) coroutine.wrap(function()
	...
	local Buy = Instance.new("TextButton", Frame) Buy.Name = "Buy" Buy.Position = UDim2.new(0, 150, 0, 170) Buy.Size = UDim2.new(1, -150, 0, 30) Buy.BackgroundColor3 = BuyColor Buy.BackgroundTransparency = 0.5 Buy.Text = BuyText Buy.Font = Settings.Font Buy.FontSize = "Size18" Buy.TextWrapped = true
...
Sold: ]]..Product.Sales..[[ 
	...
	if Product.PriceInRobux then Info.Text = Info.Text.."Robux: "..string.gsub(Product.PriceInRobux, "null", "Cannot use this currency").." " end
...
function SOUND:SyncSoundList(TellSync)
		...
		if type(LocalSoundList) == "table" then
			if #LocalSoundList > 0 then
				for _,Data in pairs(LocalSoundList) do
					SoundList[Data[1]] = {Data[2], MARKET:GetItemInfo(Data[2], true).Name}
				end
			end
		end
	end
	...
	for _,JSONID in pairs(SoundJSONs) do
		for Name,ID in pairs(CORE:GetTable(JSONID)) do
			local FullName = MARKET:GetItemInfo(ID, true).Name
			SoundList[Name] = {ID, FullName}
			GlobalSoundList[Name] = {ID, FullName}
		end
	end
...
function SOUND:PlayGlobalSound(ID, HintSoundName)
	SOUND:StopGlobalSound()
	if HintSoundName == nil then HintSoundName = true end
	local SoundID = ID
	local SoundName = nil
	local Pitch = 1
	...
	if tonumber(SoundID) then
		...
		Sound = SOUND:MakeSound(Server.Workspace, SoundID, 1, Pitch, true)
		Sound:Play()
		if HintSoundName == true then
			for _,Player in pairs(Server.Players:GetPlayers()) do
				GUI:SendHint(Player, "Playing sound: "..SoundName, 5)
			end
		end
	end
end
...
function GUI:CreateForm(Player, TitleText, Size, Position, IconID, ShowFormButtons, ColorScheme, TransparencyScheme, AutoOffset, TweenPosition, NoModal, NoScrollFrame)
	...
	local X = Instance.new("ImageButton", FormBar) X.Name = "X" X.Position = UDim2.new(1, -30, 0, 5) X.Size = UDim2.new(0, 25, 0, 25) X.BackgroundTransparency = 1 X.Image = "http://www.roblox.com/asset/?id=275572394"
	...
	local IconHolder = Instance.new("ImageLabel", FormBar) IconHolder.Name = "IconHolder" IconHolder.Position = UDim2.new(0, 5, 0, 5) IconHolder.Size = UDim2.new(0, 35, 0, 25) IconHolder.BackgroundTransparency = 1 IconHolder.Image = "http://www.roblox.com/asset/?id=276002222"
	...
	local TitleBegin = Instance.new("ImageLabel", FormBar) TitleBegin.Name = "TitleBegin" TitleBegin.Position = UDim2.new(0, 30, 0, 5) TitleBegin.Size = UDim2.new(0, 35, 0, 25) TitleBegin.BackgroundColor3 = Color3.new(1, 1, 1) TitleBegin.BackgroundTransparency = 1 TitleBegin.Image = "http://www.roblox.com/asset/?id=275589160"
	local TitleEnd = Instance.new("ImageLabel", FormBar) TitleEnd.Name = "TitleEnd" TitleEnd.Position = UDim2.new(1, -76, 0, 5) TitleEnd.Size = UDim2.new(0, 25, 0, 25) TitleEnd.BackgroundTransparency = 1 TitleEnd.Image = "http://www.roblox.com/asset/?id=275589154" 
	...
	if Settings.EnableTaskBar then
		Minus = Instance.new("ImageButton", FormBar) Minus.Name = "Minus" Minus.Position = UDim2.new(1, -58, 0, 5) Minus.Size = UDim2.new(0, 35, 0, 25) Minus.BackgroundTransparency = 1 Minus.Image = "http://www.roblox.com/asset/?id=275589157"
	else
	...
	local function CloseForm()
		if IsClosed == false then
			...
			IsClosed = true
			ypcall(function() Dragger:TweenPosition(UDim2.new(0.5-(Size.X.Scale/2),-Size.X.Offset/2,1,Size.Y.Offset + 20), "Out", "Sine", 0.7, true, function() wait(1) SG:Destroy() end) end)
			Closed:Fire()
		end
	end
	...
	local function MinimizeForm()
		if IsActive.Value == true then
			...
			ypcall(function() Dragger:TweenPosition(UDim2.new(0.5-(Size.X.Scale/2),-Size.X.Offset/2,-Size.Y.Scale,-Size.Y.Offset - 90), "Out", "Sine", 0.7, true) end)
		end
	end
	...
	local function RestoreForm()
		if IsActive.Value == false then
			IsActive.Value = true
			Restored:Fire()
			ypcall(function() Dragger:TweenPosition(MinimizePos, "Out", "Back", 0.7, true) end)
		end
	end
	
	if X then X.MouseButton1Click:connect(CloseForm) end
	if Minus then Minus.MouseButton1Click:connect(MinimizeForm) end
	
	Close.OnInvoke = CloseForm
	Minimize.OnInvoke = MinimizeForm
	Restore.OnInvoke = RestoreForm
	
	if TweenPosition then Dragger:TweenPosition(Position, "Out", "Back", 0.7, true) end
...
function GUI:SendMessage(Player, TitleText, BodyText, IconID, Time, AutoTime)
	if not Player then return nil end
	...
	if Frame then
		...
		local Said = false
		local Speak = Instance.new("ImageButton", FormBar) Speak.Name = "Speak" Speak.Position = UDim2.new(1, -87, 0, 5) Speak.Size = UDim2.new(0, 35, 0, 25) Speak.BackgroundColor3 = Color3.new(1, 1, 1) Speak.BackgroundTransparency = 1 Speak.Image = "http://www.roblox.com/asset/?id=276195369"
		if Settings.EnableTaskBar then
		...
		Speak.MouseButton1Down:connect(function()
			if not Said then
				Said = true
		...
		if tonumber(Time) then coroutine.wrap(function()
			...
			Stop.MouseButton1Down:connect(StopTimer)
			for i = Time,0,-1 do
				for ii = 1,10 do
					if Alive == true then
						Ticker.Text = i
...
function GUI:FullMessage(Player, TitleText, BodyText, IconID, Error)
	...
	if not Player:FindFirstChild("PlayerGui") then return nil end
	BodyText = string.sub(STRING:FormatReplace(Player, BodyText),1,1000)
	local TextColor = GUI:GetColor()
	if Error then TextColor = Color3.new(1,0,0) end
	
	local Frame, Close, Closed = GUI:CreateForm(Player, TitleText, UDim2.new(0.3,0,0.3,0), nil, IconID)
	...
	return Frame, Close, Closed
end

function GUI:SendHint(Player, Text, Time) coroutine.wrap(function()
	Text = STRING:FormatReplace(Player, string.sub(Text,1,100))
	if not Time then Time = 5 end
	local SG,TypeAction,Num = Instance.new("ScreenGui"),nil,0 SG.Name = "LuaMod".."".."elM".."aker's Admin Hint"
	local Frame = Instance.new("Frame", SG) Frame.Name = "Hint Frame" Frame.Position = UDim2.new(0,0,0,-80) Frame.Size = UDim2.new(1,0,0,30) Frame.BackgroundColor3 = GUI:GetColor() Frame.BackgroundTransparency = Settings.TransparencyScheme
	local Body = Instance.new("TextLabel", Frame) Body.Name = "Body" Body.ZIndex = 2 Body.Text = "" Body.BackgroundTransparency = 1 Body.Size = UDim2.new(1,0,1,0) Body.Font = "ArialBold" Body.FontSize = "Size18" Body.TextColor3 = GUI:ContrastColor()
	...
	wait(0.8)
	TypeAction = Server.RS.Stepped:connect(function()
		Num = Num + 1
	...
	if Frame and Frame.Parent then
		Frame:TweenPosition(UDim2.new(0,0,0,-80), "Out", "Quint", 0.7, true, function() SG:Destroy() end)
	end
end)() end
...
function GUI:ListGui(Player, Title, List, IconID, Clickable, AutoNumber, PreSearch, BackgroundColor)
	...
	local function ShowResults(Key)
		...
		if ScrollScript then ScrollScript.Disabled = true ScrollScript:Destroy() end
		...
		for _,Data in pairs(List) do
			...
			local String = STRING:Defilter(tostring(Data))
			if String then
				local ImageIcon = nil
				if type(Data) == "table" then
					String = Data[1]
				...
				if string.find(string.lower(String), string.lower(Key)) then
					...
					elseif ClientInfo[Player.Name] and ClientInfo[Player.Name].TouchScreen ~= true then
						Content.MouseButton1Down:connect(function() Clicked:Fire(String) end)
					end
	...
	SearchBar.Changed:connect(function(Prop)
		if Prop == "Text" then ShowResults(SearchBar.Text) Frame.CanvasPosition = Vector2.new(0,0) end
	end)
			...
			if ValueType then
				...
				if ValueType == "string" or ValueType == "number" then
					if Writable == true then
						...
						Input.Changed:connect(function(Prop) if Prop == "Text" then
...
function GUI:CreateTaskBar(Player)
	...
	if Settings.EnableAdminMenu then
		...
		coroutine.wrap(function() wait(1) MenuButton.MouseButton1Click:connect(function()
			if Open == false then
				...
				local Dnt = Dnt:Clone() Dnt.Parent = Donate Dnt.Position = UDim2.new(0,0,1,-90) Dnt.Text = "20 Robux" Dnt.BackgroundColor3 = Color3.new(0,1,0) Dnt.MouseButton1Down:connect(function() DonateMoney("20R", Dnt) end)
				local Dnt = Dnt:Clone() Dnt.Parent = Donate Dnt.Position = UDim2.new(0,0,1,-60) Dnt.Text = "100 Robux" Dnt.BackgroundColor3 = Color3.new(0,1,0) Dnt.MouseButton1Down:connect(function() DonateMoney("100R", Dnt) end)
				local Dnt = Dnt:Clone() Dnt.Parent = Donate Dnt.Position = UDim2.new(0,0,1,-30) Dnt.Text = "500 ROBUX!" Dnt.BackgroundColor3 = Color3.new(1,0,0) Dnt.FontSize = "Size18" Dnt.MouseButton1Down:connect(function() DonateMoney("500R", Dnt) end)
				local PlayerAdmin, PlayerRank = RANK:IsAdmin(Player)
...
function GUI:ShowCommand(Player, CommandTrigger)
	if not Player then return nil end
...
Abusable command: ]]..STRING:BoolString(CommandData.Abusable)
	...
	GUI:FullMessage(Player, TitleText, BodyText, "Search", not CanExe)
end
...
function GUI:CreateHUD(Speaker)
	...
	local function AddHUD(Player, Char)
		...
		local HUD = Instance.new("ImageLabel", PlayerHUD) HUD.Name = "HUD" HUD.Size = UDim2.new(1, 0, 1, 0) HUD.BackgroundColor3 = Color3.new(1, 1, 1) HUD.BackgroundTransparency = 1 HUD.Image = "rbxassetid://"..RankHUD[Rank]
		
		Char.ChildRemoved:connect(function(Obj) if Obj.Name == "Head" then
			PlayerHUD:Destroy()
			Dead = true
			return nil
		end end)
		...
		coroutine.wrap(function() repeat wait()
			if Player and Player.Character and Player.Character:FindFirstChild("Head") and Speaker and Speaker.Character and Speaker.Character:FindFirstChild("Head") then
				if (Player.Character.Head.Position - Speaker.Character.Head.Position).magnitude < 20 then
					if HUD.Rotation <= 0 then
						coroutine.wrap(function()
							for i = 1,45,4 do
								HUD.Rotation = i
								wait()
...
function GUI:CreateMessagePrompt(Player, Title, ImageID, PreAdd, SpeakerEvents)
	...
	Frame.CanvasSize = UDim2.new(0,500,0,300)
	...
	local Send = Instance.new("TextButton", Frame.Parent) Send.Position = UDim2.new(1, -50, 1, -30) Send.Size = UDim2.new(0, 50, 0, 30) Send.BackgroundColor3 = Color3.new(1, 1, 1) Send.BackgroundTransparency = 0.5 Send.Text = "Enter" Send.Font = Settings.Font Send.FontSize = "Size14"
	
	local function AddLine(Speaker, MessageText, MessageColor)
		MessageText = STRING:Defilter(MessageText)
		if MessageColor == true then MessageColor = Color3.new(1,0,0) end
		if not MessageColor then MessageColor = GUI:ContrastColor() end
		for Num,Bound in pairs(Elements) do
			Bound.Position = Bound.Position + UDim2.new(0,0,0,-20)
			if Bound.Position.Y.Offset <= -1020 then Bound:Destroy() end
		end
		local Bounds = Instance.new("Frame", Frame) Bounds.Name = "Bounds" Bounds.Position = UDim2.new(0, 0, 1, -20) Bounds.Size = UDim2.new(1, 0, 0, 0) Bounds.BackgroundTransparency = 1
		...
		CORE:ExecuteResource("ReposGui", Bounds)
		table.insert(Elements, Bounds)
	end
	
	if PreAdd then
		for Num,Add in pairs(PreAdd) do
			if Num >= #PreAdd - 20 then
	...
	local function FireSendPressed()
		if ChatBox.Text ~= "" and ChatBox.Text ~= "Click here or press the '-' key" then
			SendPressed:Fire(ChatBox.Text)
			ChatBox.Text = "Click here or press the '-' key"
		end
	end
	...
	Send.MouseButton1Down:connect(FireSendPressed)
	EnterPressed.OnServerInvoke = FireSendPressed
...
function GUI:PromptMessageWithButtons(Player, Title, BodyText, IconID, Buttons, Time, ShowFormButtons)
	...
	for Num,Select in pairs(Buttons) do
		...
		Button.MouseButton1Down:connect(function() ClosePrompt(Name) end)
	end
...
function GUI:Vote(Speaker, Players, Text, Time)
	...
	for _,Player in pairs(Players) do
		...
		if CanVote == true then
			coroutine.wrap(function()
				local Answer = GUI:PromptMessageWithButtons(Player, "Vote", Text, nil, {"Yes", "No"}, Time)
...
function GUI:CommandBar(Player)
	...
	local function ExecuteText()
		CORE:Chatted(CommandBox.Text, Player, true)
	...
	local function AddSuggestion(Text)
		...
		Suggestion.MouseButton1Down:connect(function()
	...
	CommandBox.MouseEnter:connect(function()
	...
	Execute.MouseButton1Down:connect(ExecuteText)
	EnterPressed.OnServerInvoke = ExecuteText
	
	CommandBox.Changed:connect(function(Prop) if Prop == "Text" then
...
function GUI:SendChat(Player, Data) coroutine.wrap(function()
	if not Player then return nil end
...
Commands = { -- can't make local
	...
	[{"m", "msg", "message"}] = {"string", "Sends everyone in a server a message of string", 1, "Member", false, function(Speaker, Rank, Arguments)
		for _,Player in pairs(Server.Players:GetPlayers()) do
			GUI:SendMessage(Player, "Message from "..Speaker.Name, Arguments[1], "Message", 10, true)
		end
	end};
	...
	[{"sm", "smsg", "smessage", "systemm", "systemmsg", "systemmessage"}] = {"string", "Sends everyone in a server a message of string as a system message", 1, "Admin", false, function(Speaker, Rank, Arguments)
		for _,Player in pairs(Server.Players:GetPlayers()) do
			GUI:SendMessage(Player, "MAK".."ERMODELLU".."A'S ADMIN SYSTEM MESSAGE", Arguments[1], 132769569)
		end
	end};
	...
	[{"pm", "personalmessage", "personalmsg", "personalm", "pmessage", "pmsg"}] = {"player string", "Sends the particular player a message of string", 2, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			GUI:SendMessage(Player, "Message from "..Speaker.Name, Arguments[2], "Message", 20)
		end
	end};
	...
	[{"cm", "chatmessage", "chatmsg"}] = {"string", "Sends everyone in a server a chatted message of string", 1, "Admin", false, function(Speaker, Rank, Arguments)
		for _,Player in pairs(Server.Players:GetPlayers()) do
			GUI:SendChat(Player, {Text = "[ MML'S ADMIN ]: "..Speaker.Name..": "..Arguments[1], FontSize = Enum.FontSize.Size24})
		end
	end};
	...
	[{"pcm", "pchatmessage", "pchatmsg", "personalcm", "personalchatmessage", "personalchatmsg"}] = {"player string", "Sends player the chatted message of string", 2, "Admin", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			GUI:SendChat(Player, {Text = "[ MML'S ADMIN ]: "..Speaker.Name..": "..Arguments[2], FontSize = Enum.FontSize.Size24})
		end
	end};
	...
	[{"commands", "cmds", "commandlist", "cmdlist"}] = {"", "Shows a list of all the commands", 0, "Non-Admin", false, function(Speaker, Rank, Arguments)
		...
		for CommandNames,Data in pairs(Commands) do
			if not Data.Hidden then
				...
				if Data[4] == "Admin" then Add = "WHITE " end
				if Data[4] == "Member" then Add = "BLUE " end
				if Data[4] == "Owner" then Add = "YELLOW " end
		...
		local Clicked = GUI:ListGui(Speaker, "Command List", Formatted, 98616974, true)
		Clicked.Event:connect(function(Text)
			GUI:ShowCommand(Speaker, StorageText[Text])
		end)
	end};
	...
	--[[
	[{"newserver", "makeserver"}] = {"", "Opens new server on the current game (HTTP)", 0, "Owner", false, function(Speaker, Rank, Arguments)
	...
	[{"remotenewserver", "remotemakeserver", "rns"}] = {"number(ID)", "Opens new server on the ID of the game (HTTP)", 1, "Owner", false, function(Speaker, Rank, Arguments)
	...
	[{"script", "exe"}] = {"string", "Will execute a script with global admin variables", 1, "Owner", false, function(Speaker, Rank, Arguments)
		CORE:Execute(Arguments[1], Speaker)
	end};
	
	[{"exehttp", "httpexe", "remoteexe", "plugin", "pluginhttp", "httpplugin", "remoteplugin"}] = {"string(URL)", "Will execute a script with global admin variables from the link (HTTP)", 1, "Owner", false, function(Speaker, Rank, Arguments)
		local Script = REMOTE:GetURL(Arguments[1], Player)
		if Script then
			CORE:Execute(Script, Speaker)
		end
	end};
	
	[{"ping", "url", "link", "http"}] = {"string(URL)", "Pings string which is a URL (HTTP)", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Time = tick()
	...
	[{"h", "hint"}] = {"hint string", "Gives everyone in the server a hint string", 1, "Member", false, function(Speaker, Rank, Arguments)
		for _,Player in pairs(Server.Players:GetPlayers()) do
			GUI:SendHint(Player, Arguments[1])
		end
	end};
	
	[{"murica", "merica", "america"}] = {"murica player", "You'll have to find out yourself (HTTP)", 1, "Admin", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		local Debt = nil
		for _,Player in pairs(Players) do
			if not Debt then
	...
	[{"hud", "target", "profiler"}] = {"player", "Opens a gui HUD that allows player to see player ranks though color", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
	...
	[{"nohud", "notarget", "noprofiler", "unhud", "untarget", "unprofiler"}] = {"player", "Removes HUD Gui if it's on the player", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			if Player:FindFirstChild("PlayerGui") then
	...
	[{"debug"}] = {"player", "Respawns player wherever they are at and fixes camera", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
	...
	[{"search", "searchsound", "searchsounds", "searchaudio", "searchmusic", "ss"}] = {"string", "Searches the catalog for the sound string (HTTP)", 1, "Member", false, function(Speaker, Rank, Arguments)
	...
	[{"ranked", "ranks", "ranklist"}] = {"", "Lists all the players ranked in the admin", 0, "Member", false, function(Speaker, Rank, Arguments)
		...
		GUI:ListGui(Speaker, "Exile List", TableStorage, 21341765)
	end};
	...
	[{"addsound", "addaudio", "addmusic"}] = {"num(ID) string", "Will add the song of num(ID) named string to the custom sound list of the game", 2, "Admin", false, function(Speaker, Rank, Arguments)
		...
		if tostring(Arguments[1]) then
			...
			if Request.AssetTypeId == 3 then
				...
				SoundList[Arguments[2]] = {Arguments[1], Request.Name}
	...
	[{"hat"}] = {"player ID", "Gives player the hat with the ID of ID", 2, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			if Player.Character then
				CORE:Insert(Arguments[2], Player.Character, true, {"Hat"})
			end
		end
	end};
	...
	[{"abuselist", "abusablecommands", "abusecmds", "ac"}] = {"", "Shows a list of abusable commands", 0, "Member", false, function(Speaker, Rank, Arguments)
	...
	[{"nofly", "unfly", "clip", "nonoclip", "unnoclip"}] = {"player", "Removes fly or noclip effect", 1, "Member", false, function(Speaker, Rank, Arguments)
		...
		for _,Player in pairs(Players) do
			if Player.Character then
				if Player.Character:FindFirstChild("StopLMMFly") then
					Player.Character.StopLMMFly:InvokeClient(Player)
				end
			end
		end
	end};
	...
	[{"clearcache", "clearc", "cc"}] = {"", "Clears cached objects such as market info and http local scripts", 0, "Owner", false, function(Speaker, Rank, Arguments)
		ScriptCache,MarketCache,CharacterCache,PackageCache = {},{},{},Packages
	...
	[{"ghost", "ghostify", "spook", "spooky"}] = {"player", "Makes player's character a ghost", 1, "Member", false, function(Speaker, Rank, Arguments)
		...
		for _,Player in pairs(Players) do
			if Player.Character then
				...
				CORE:ScanItems(Player.Character, {"Texture", "Decale"}, function(Obj)
					Obj.Transparency = 0.5
				end)
			end
		end
	...
	[{"nohats", "nohat"}] = {"player", "Removes player's hats", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			if Player.Character then
				for _,Obj in pairs(Player.Character:GetChildren()) do
	...
	[{"nolight", "nolamp", "unlight", "unlamp"}] = {"player", "Makes player and a small radius in from of whom to light up as if there was a head light", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			if Player.Character and Player.Character:FindFirstChild("Torso") then
				local Lights = {}
	...
	[{"searchg", "searchgear"}] = {"string", "Searches the catalog for the gear string (HTTP)", 1, "Member", false, function(Speaker, Rank, Arguments)
		local AbuseMode = false
		if Rank == "Member" then AbuseMode = Settings.DisableAbuse end
	...
	[{"searchh", "searchhat"}] = {"string", "Searches the catalog for the hat string (HTTP)", 1, "Member", false, function(Speaker, Rank, Arguments)
		MARKET:ShowSearch(Speaker, 3, Arguments[1], function(ID)
			if Speaker.Character then
				CORE:Insert(ID, Speaker.Character, true, {"Hat"})
			end
		end, 9)
	end};
	
	[{"searchp", "searchpack", "searchpackage"}] = {"string", "Searches the catalog for the package string (HTTP)", 1, "Member", false, function(Speaker, Rank, Arguments)
		MARKET:ShowSearch(Speaker, 4, Arguments[1], function(ID)
			if Speaker.Character then
	...
	[{"package", "pack", "morph"}] = {"player num(ID)", "Gives player the package with num(ID), a few packages don't need http service because they're in the cache, but most do (HTTP)", 2, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
	...
	[{"insert", "model", "ins"}] = {"num(ID)", "Allows player to insert num(ID) as a model(Only works if the model is owned by the owner of the game or ROBLOX)", 1, "Member", false, function(Speaker, Rank, Arguments)
		if Speaker:FindFirstChild("Backpack") and tonumber(Arguments[1]) then
			local Model = CORE:Insert(tonumber(Arguments[1]), game:GetService("ReplicatedStorage"))
	...
	[{"muslim", "islam", "isis"}] = {"player", "Turns player into a muslim", 1, "Member", false, function(Speaker, Rank, Arguments)
		...
		for _,Player in pairs(Players) do 
			if Player.Character and Player.Character:FindFirstChild("Head") and Player.Character.Head:FindFirstChild("face") then
				...
				Player.Character.Head.face.Texture = "http://www.roblox.com/asset/?id=110287880"
	...
	[{"chat"}] = {"player (color) string", "Makes player chat string. If color is available the color will be set to it", 2, "Member", false, function(Speaker, Rank, Arguments)
		...
		for _,Player in pairs(Players) do
			if Player.Character and Player.Character:FindFirstChild("Head") then
				game:GetService("Chat"):Chat(Player.Character.Head, String, Color)
			end
		end
	end};
	...
	[{"downloadlogs", "downloadhttplogs", "getlogs", "gethttplogs"}] = {"", "Sends string as a packet to the owner remotely", 0, "Admin", false, function(Speaker, Rank, Arguments)
	...
	[{"removenote"}] = {"player string", "Removes any note starting with string", 1, "Admin", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
	...
	[{"shownotes", "shownote", "loadnotes", "loadnote"}] = {"player", "Shows the saved notes of player", 1, "Member", false, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		local DataSet = {}
		for _,Player in pairs(Players) do
			local ExistingData = Player:LoadInstance("Data")
	...
	[{"vipadmin", "adminvip"}] = {"", "Prompts speaker the VIP item to grant Admin level permissions", 0, "Non-Admin", false, function(Speaker, Rank, Arguments)
		if Settings.VIPAdminID <= 1 then
			...
			SendMessage(Speaker, "No Admin VIP", "The creator ("..GameOwner..") did not add a admin rank VIP to Make".."rModelLua's Admin", 7)
		end
	end};
	...
	[{"vipmember", "membervip"}] = {"", "Prompts speaker the VIP item to grant Member level permissions", 0, "Non-Admin", false, function(Speaker, Rank, Arguments)
		if Settings.VIPMemberID <= 1 then
			...
			SendMessage(Speaker, "No Member VIP", "The creator ("..GameOwner..") did not add a member rank VIP to Make".."rModelLua's Admin", 7)
		end
	end};
	...
	[{"define", "lookup", "definition"}] = {"string", "Looks up the word on webster's online dictionary (HTTP)", 1, "Member", false, function(Speaker, Rank, Arguments)
	...
	[{"plugins", "getplugins", "pluginlist"}] = {"", "Gets player's position and prompts to add it to waypoints", 0, "Admin", false, function(Speaker, Rank, Arguments)
		if Plugins then
			local List = {}
	...
	[{"truck", "car", "runover"}] = {"player", "Runs player over with a truck", 1, "Member", true, function(Speaker, Rank, Arguments)
		local Players = STRING:Scan(Arguments[1], Speaker)
		for _,Player in pairs(Players) do
			if Player.Character and Player.Character:FindFirstChild("Torso") and Player.Character:FindFirstChild("Humanoid") then
				local Dead = false
	...
	[{"telljoke", "showjoke", "tj", "sj"}] = {"player", "Tells player a random joke (HTTP)", 1, "Admin", true, function(Speaker, Rank, Arguments)
	...
	[{"joke", "laugh"}] = {"player", "Tells speaker a random joke (HTTP)", 0, "Member", true, function(Speaker, Rank, Arguments)
		local Joke = REMOTE:DownloadJoke(Speaker)
...
function CORE:Start(Player) coroutine.wrap(function()
	...
	if Player then
		...
		Player.Chatted:connect(function(Message) if Message ~= "" then
			CORE:Log(ChatLogs, Speaker, Message)
...
ExecuteCode.OnInvoke = function(Source, Speaker)
	if type(Source) == "string" then
		CORE:Execute(Source, Speaker)
	end
end
]]

RankPlayerLocal.OnServerInvoke = function(Player)
	wait()
	local Request = PendingRequests[Player]
	if Request then
		if math.abs(tick() - Request[2]) <= 1 then -- Expired brah
			if Settings.Ranks[Request[1]] then
				PendingRequests[Player] = nil
				table.insert(Settings.Ranks[Request[1]], Player.Name)
				GUI:TellAdmin(Player)
			end
		end
	end
...
RankPlayerServer.OnInvoke = function(Player, Rank)
	if not RANK:IsAdmin(Player) then
		PendingRequests[Player] = {Rank, tick()}
	end
end

IsAdmin.OnServerInvoke = function(Player)
	local IsFullAdmin = false
	for _,PName in pairs(Settings.Ranks["Owner"]) do
		if PName == Player.Name then IsFullAdmin = true end
	end
	for _,PName in pairs(Settings.Ranks["Admin"]) do
		if PName == Player.Name then IsFullAdmin = true end
	end
...
AddExploit.OnServerInvoke = CORE.AddExploit
AddExploitServer.OnInvoke = CORE.AddExploit

KickClient.OnServerInvoke = function(PlayerClient)
	PlayerClient:Kick()
end

ConvertTextToSound.OnServerInvoke = function(_, Text, Parent)
	return SOUND:SayConvertedText(SOUND:ConvertText(Text), Parent)
end
...
for Cmds,Data in pairs(Commands) do -- check if commands are abusable
...
if not IsStudio then coroutine.wrap(function()
	ypcall(function() RemoteData = REMOTE:Decode(REMOTE:GetURL(CORE.Domain.."/RemoteData.txt")) end)
	coroutine.wrap(function()
...
coroutine.wrap(function() while true do
	local ThisDate = REMOTE:GetURL(CORE.Domain.."/APIs/DateTime.php")
